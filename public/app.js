(function () {
  ...
  function renderFeed(items) {
    $("#feed").innerHTML = items.length
      ? items.map(function (i) { return "<li>" + i.text + "<time>" + timeAgo(i.created_at) + "</time></li>"; }).join("")
  ...
  function renderTopChannels(items) {
    ...
    $("#topChannels").innerHTML = items.length
      ? items.map(function (c) {
          ...
          return '<div class="row"><span>#' + c.name + '</span><div class="track"><div class="fill" style="width:' + pct + '%"></div></div><span class="val">' + c.messages + '</span></div>';
        }).join("")
  ...
  function renderVoice(channels) {
    $("#vcList").innerHTML = channels.map(function (c) {
      var body = c.members.length
        ? '<div class="vc__members">' + c.members.map(function (m) { return '<span class="avatar" title="' + m.name + '">' + m.name.slice(0, 2).toUpperCase() + '</span>'; }).join("") + '</div>'
        : '<span class="vc__empty">— empty —</span>';
      return '<div class="vc"><div class="vc__head"><b>' + c.name + '</b><span>' + c.members.length + ' in call</span></div>' + body + '</div>';
    }).join("");
  }

  function renderRoles(roles) {
    $("#rolesList").innerHTML = roles.map(function (r) {
      return '<div class="role-row"><span class="role-swatch" style="background:' + r.color + '"></span><span class="name">' + r.name + '</span><span class="count">' + r.count + ' members</span></div>';
    }).join("");
  }
  ...
  function renderMod(flags) {
    ...
    $("#modQueue").innerHTML = flags.length
      ? flags.map(function (f) {
          return '<div class="mod-item" data-id="' + f.id + '"><div class="meta">' + f.author_tag + ' · ' + f.reason + ' · ' + timeAgo(f.created_at) + '</div><p>' + f.content.replace(/</g, "&lt;") + '</p><div class="actions"><button data-act="approve">approve</button><button data-act="remove">remove message</button></div></div>';
        }).join("")
  ...
  function populateChannelSelect(channels) {
    ...
    sel.innerHTML = channels.map(function (c) { return '<option value="' + c.id + '">#' + c.name + '</option>'; }).join("");
  }
